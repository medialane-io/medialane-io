        ...
        <div className="flex flex-col sm:flex-row gap-2 w-full pt-1">
          ...
          {mintedTicketId && (
            <Button asChild className="flex-1 bg-teal-600 hover:bg-teal-700 text-white">
              <Link href={`${collectionHref("STARKNET", contract)}`}>View collection</Link>
            </Button>
          )}
        </div>
      </TransactionDialog>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-4">
                <div
                  ...
                  onClick={() => !imageUploading && document.getElementById("mint-ticket-file-input")?.click()}
                  onKeyDown={(e) => { if (e.key === "Enter") document.getElementById("mint-ticket-file-input")?.click(); }}
                ...
                <div className="space-y-1.5">
                  <input id="mint-ticket-file-input" type="file" accept="image/*" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) handleImageSelect(f); }} />
                  <Button type="button" variant="outline" size="sm" disabled={imageUploading} onClick={() => document.getElementById("mint-ticket-file-input")?.click()}>
                    {imageUploading ? <><Loader2 className="h-3.5 w-3.5 mr-1.5 animate-spin" />Uploading…</> : imagePreview ? "Change" : "Upload image"}
                  </Button>
                  {imagePreview && (
                    <button type="button" onClick={handleImageClear} className="flex items-center gap-1 text-xs text-muted-foreground hover:text-destructive transition-colors">
            ...
            <button
              ...
              className={`w-full h-12 text-base font-semibold text-white rounded-xl flex items-center justify-center gap-2 transition-all hover:brightness-110 active:scale-[0.98] bg-teal-600 ${action.status !== "idle" || imageUploading ? "opacity-40 pointer-events-none" : ""}`}
            >
